...
def plot_model(epoch, train_loss,loss_cla,loss_sas,loss_bfa,loss_dih,loss_cor,test_loss,val_loss, 
               corsingle_mean, cormultip_mean, score_all, pred_all, label_all,
               filename):
    ...
    plt.plot(corsingle_mean, label='single_corr')
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    if epoch < 500:
        plt.xlim((0, 501))
    ...
    plt.legend()
    plt.title(
        f'best multiple mutation: {max(cormultip_mean):.4f}; single mutation: {max(corsingle_mean):.4f}',
        fontsize=16)
#     ROC_plot(score_all, label_all)

    labels_name = ['C', 'S', 'T', 'A', 'G', 'P', 'D', 'E', 'Q', 'N', 'H', 'R', 'K', 'M', 'I', 'L', 'V', 'W', 'Y', 'F']
    # print(0)

    drawconfusionmatrix = DrawConfusionMatrix(labels_name=labels_name)
    # array([0,5,0,6,2,...],dtype=int64)
    predict_np = pred_all.numpy().astype(int)
    labels_np = label_all.numpy().astype(int)
